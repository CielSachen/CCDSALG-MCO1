 ...
 * empirical-time.c
 *
 * Sample program for determining the empirical execution time. Note that the actual numerical
 * results will depend on the platform, i.e., the hardware, the compiler, the operating system.
 */
...
void algo(long int n) {
    ...
    for (long int i = 1; i <= n; ++i) {  // outer loop: this is O(n)
        ...
        for (long int j = 1; j <= n;
             j++)   // inner loop: this is O(n^2) as affected by the outer loop
            ctr++;  // this is O(n^2)
    }
