 ...
 */
void create(Stack *const stack) { stack->top = -1; }
 ...
 */
void push(Stack *const stack, const Point element) { stack->data[++stack->top] = element; }
 ...
 */
Point pop(Stack *const stack) { return stack->data[stack->top--]; }
 ...
 */
Point *top(Stack *const stack) { return &stack->data[stack->top]; }

/**
 * @brief Checks if a stack contains the maximum number of elements.
 * @param[in] stack The stack to check.
 * @return Whether the stack's top index is `MAX_STACK_SIZE`.
 */
 ...
 */
bool is_empty(const Stack *const stack) { return stack->top == -1; }
