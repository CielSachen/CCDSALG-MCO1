...
TEST(PolarAngle, ReturnsThePolarAngleOfThePoint) {
    ...
    const Point anchor_point = {0, 0};

    EXPECT_EQ(polar_angle(&point, &anchor_point), 0.92729521800161219);
}

TEST(PolarAngle, ReturnsZeroIfThePolarAngleIsNan) {
    const Point point = {3, 4};
    const Point anchor_point = {0, NAN};

    EXPECT_EQ(polar_angle(&point, &anchor_point), 0);
}
